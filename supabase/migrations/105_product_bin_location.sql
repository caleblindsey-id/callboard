-- Whse 4 (service dept) bin/shelf location for the parts pick list. Synced from
-- SynergyERP `prodloc` (Loc, filtered Whse=4), GROUP_CONCAT'd primary-first when
-- a part lives in more than one bin (e.g. "E5, E5-D"). NULL = no bin record.
--
-- Projects products.bin_location onto each queue row via the existing products
-- LEFT JOIN (pr, added migration 102). CREATE OR REPLACE VIEW can only APPEND
-- columns, so bin_location is added LAST in both UNION branches (after pulled_by,
-- the migration-104 tail).
