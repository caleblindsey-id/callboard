-- Parts-queue Review step support. Rebuilt from migration 096, with two changes:
--
-- 1) Project the stock signal + triage audit onto each queue row:
--      qty_on_hand / qty_on_po  <- products catalog (LEFT JOIN by product_number;
--                                  null for manual / non-catalog parts)
--      triaged_by/at, triage_reason, qoh_at_triage, qopo_at_triage <- the
--                                  parts_requested JSONB element
--    CREATE OR REPLACE VIEW can only APPEND columns, so all seven are added LAST
--    in both UNION branches, in the same position (after covered_by_agreement).
--
-- 2) Extend the service-ticket estimate gate: a tech's new request now lands in
--    'pending_review' (was 'requested'), so hide BOTH 'requested' and
--    'pending_review' service parts while the estimate is still open/estimated —
--    you don't triage sourcing on a part the customer hasn't approved yet.
--
-- security_invoker=on preserved (RLS: staff see all rows, techs see only their
-- own tickets). The products LEFT JOIN is 1:1 (products.number is unique per
-- ProdCode) so it never multiplies rows.
