...
import { bulkSoftDeleteServiceTickets } from '@/lib/db/service-tickets'
import { getCurrentUser, RESET_ROLES } from '@/lib/auth'
import { createClient } from '@/lib/supabase/server'
import { partsOnOrder } from '@/lib/parts'
import type { PartRequest } from '@/types/service-tickets'

// Mirror of src/app/api/tickets/bulk-delete/route.ts for service tickets:
// soft-delete (parity with the single-ticket DELETE route, migration 082),
// with the same all-or-nothing parts-on-order guard.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const deleted = await bulkSoftDeleteServiceTickets(ticketIds, user.id)

    return NextResponse.json({ count: deleted.length })
  } catch (err) {
    console.error('service-tickets/bulk-delete error:', err)
    return NextResponse.json(
