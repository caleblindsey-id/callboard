...
Each row also has the optional **Synergy Order #** field next to the invoice number — the same one from **Ready to Export**, so anything you entered before exporting shows here too. Tap **+ Synergy Order #** to jot down (or correct) the Synergy order number you're billing against so it stays on screen while you track down the matching invoice — then key that invoice number in. It's only a reference and never blocks **Mark Billed**.
